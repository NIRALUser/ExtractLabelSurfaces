...
int CreateSurfaceLabelFiles(std::string vtkFile, std::string labelNumberInfo)
{
    ...
    for(mit=labelMap.begin(),mend=labelMap.end() ; mit!=mend ; ++mit)
    {
        ...
        int nbPolys=0;
        //Vector string to write in output files
        std::vector<std::string> outputVect ;

        //Iterateur outputVect;
        //Vector iterator
        std::vector<std::string>::iterator vit, vend ;
        ...
        outputVect.push_back("#!ascii - generated by CreateLabelFiles project \n");
        std::string infoPolyData = IntToString(mit->second.size()) + " " ;
        outputVect.push_back(infoPolyData);
        //Write points coord
        for(lit=mit->second.begin(),lend=mit->second.end() ; lit!=lend ; ++lit)
        {
            ...
            linePoints += "0\n" ;
            outputVect.push_back(linePoints);
        ...
        for(vtkIdType j=0 ; j<polyData->GetNumberOfCells() ; j++)
        {
            ...
            int polyFound=0 ;
            std::list<int>::const_iterator iter[nbCompoCell] ;
            for(int i = 0; i < nbCompoCell ; i++)
            {
                cellValue[i]=cellIdList->GetId(i) ;
                iter[i]=find(mit->second.begin(),mit->second.end(),cellValue[i]) ;
                if(iter[i]!=mit->second.end())
                {
                    polyFound++ ;
                }
            }
            if(polyFound==nbCompoCell)
            {
                ...
                for(int i = 0; i < nbCompoCell ; i++)
                {
                    std::string cell = IntToString(std::distance(lit, iter[i])-1) ;
                    lineCells += cell + " ";
                }
                lineCells += "0\n" ;
                outputVect.push_back(lineCells);
            }
        }
        //Insert number of polygons in the second line
        ...
        if(outputFile.good())
        {
            for(vit=outputVect.begin(),vend=outputVect.end() ; vit!=vend ; ++vit)
            {
                outputFile << *vit ;
            }
        }
        ...
        outputFile.close() ;
        std::cout<<"Surface ASCII file: "<<labelName.c_str()<<" created"<<std::endl ;
    }

...
int CreateSurfaceLabelFiles(std::string vtkFile, std::string labelNumberInfo)
{
    ...
    for(mit=labelMap.begin(),mend=labelMap.end() ; mit!=mend ; ++mit)
    {
        ...
        if(outputFile.good())
        {
            ...
            outputFile << mit->second.size() << " nbPolyFound\n" ;
            //Write points coord
            ...
            for(vtkIdType j=0; j<polyData->GetNumberOfCells();j++)
            {
                ...
                if(polyFound==nbCompoCell)
                {
                    ...
                    for(int i = 0; i < nbCompoCell ; i++)
                    {
                        outputFile << std::distance(lit, iter[i])-1 << " " ;
                    }
                    outputFile << "0\n" ;
        ...
        //Re-open outputFile to change numbers of polygons found.
        ...
        {
            std::cout<<"Error in "<<labelName.c_str()<<" file - couldn't be open"<<std::endl ;
            return EXIT_FAILURE ;
        }
        readFile.close() ;
